...
require_once 'config.php';
 ...
 function neighbours() {
  global $db, $ROLES, $HARDWAREMODEL;

  $result = $db->query("SELECT node_id, long_name, short_name, neighbours, UNIX_TIMESTAMP(updated_at) AS updated_at, UNIX_TIMESTAMP(neighbours_updated_at) AS neighbours_updated_at, role, hardware_model, battery_level, voltage, air_util_tx, channel_utilization, latitude, longitude, altitude, temperature, relative_humidity, barometric_pressure, region FROM nodes");
  $data = [];
  while($row = $result->fetch()) {
   $data[$row["node_id"]] = [
    ...
    "neighbours" => $row["neighbours"] !== null ? json_decode($row["neighbours"], true) : null,
 ...
 function getgraphdata() {
  ...
  $result = $db->query("SELECT UNIX_TIMESTAMP(updated_at) AS updated_at, `from`, `to`, route, route_back, snr_towards, snr_back, want_response FROM traceroutes ORDER BY updated_at ASC");
  if($result) {
   while($row = $result->fetch()) {
  ...
  if (!isset($db) || !($db instanceof PDO)) {
    ...
    echo json_encode(["nodes" => [], "links" => [], "error" => "Database connection failed."]);
    exit;
  }
  ...
  try {
    ...
    echo json_encode(["nodes" => [], "links" => [], "error" => "An error occurred while fetching mesh data."]);
  }
?>
