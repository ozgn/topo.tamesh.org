<html lang="tr-TR">
 ...
 <body>
  <input type="text" class="search-bar" placeholder="Düğüm Ara..." />
  <div class="results"></div>
  <svg></svg>
  <script>
   function getShortnameColor(role, region) {
    switch(role) {
     case "CLIENT":
      if (region === "2") return "#0a0"; // EU_433
      else if (region == "3") return "#FF7518"; // EU_868
      else return "#080"; // varsayılan renk
     case "CLIENT_MUTE":
      return "#00f";
     case "ROUTER":
      return "#80f";
   ...
   fetch("mesh.php?_=" + new Date().getTime()).then(response => response.json()).then(data => {
    ...
    function loadMeshSimulation(selectedMesh) {
     ...
     node.append("circle")
      .attr("r", nodeSize)
      .attr("fill", d => getShortnameColor(d.role, d.region))
      .attr("opacity", 0.5);
    ...
    function showTooltip(d) {
     ...
     let tooltipHtml = "<b>Ad</b> <span class=\"shortname\" style=\"background-color: " + 
     getShortnameColor(d.role, d.region) + ";\">" + d.short_name + "</span> " + d.long_name + 
     //"<br><b>MAC</b> <span style=\"color: #ff0;\">" + d.id.toString(16).padStart(8, '0').slice(-4) + "</span> " + 
     d.id + "<br><b>Son Güncelleme</b> " + updatedDate.toLocaleString();
     ...
     if(d.region == "2") tooltipHtml += "<br><b>Bölge</b> <span style=\"background-color: #080;\">Yalnızca traceroute verisi</span>";
     if(d.region == "3") tooltipHtml += "<br><b>Bölge</b> <span style=\"background-color: #080;\">Yalnızca traceroute verisi</span>";

     if(d.neighbours_updated_at === null)
      tooltipHtml += "<br><b>Komşular</b> <span style=\"background-color: #080;\">Yalnızca traceroute verisi</span>";
     else
      tooltipHtml += "<br><b>Komşular</b> " + neighborsUpdatedDate.toLocaleString();

     if(d.role === "ROUTER_CLIENT")
      tooltipHtml += "<br><b>Rol</b> <span style=\"background-color: #800;\">ROUTER_CLIENT is deprecated</span>";
     else
      tooltipHtml += `<br><b>Rol</b> <span style="background-color: ${getShortnameColor(d.role, d.region)};">${d.role}</span>`;

     tooltipHtml += "<br><b>Donanım</b> " + d.hardware_model;

     if(d.battery_level !== null) {
      if(d.battery_level === 101)
       tooltipHtml += "<br><b>Pil</b> Plugged In";
      else
       tooltipHtml += "<br><b>Pil</b> " + d.battery_level + " %";
     }

     if(d.voltage !== null)
      tooltipHtml += "<br><b>Volt</b> " + Number(d.voltage).toFixed(2) + " V";
     ...
     if(d.temperature !== null)
      tooltipHtml += "<br><b>Sıcaklık</b> " + Number(d.temperature).toFixed(1) + " °C";
     if(d.relative_humidity !== null)
      tooltipHtml += "<br><b>Nem</b> " + Number(d.relative_humidity).toFixed(1) + " %";
     if(d.barometric_pressure !== null)
      tooltipHtml += "<br><b>Basınç</b> " + Number(d.barometric_pressure).toFixed(1) + " hPa";
    ...
    function populateSearchResults() {
     ...
     components.forEach((component, index) => {
      ...
      if(component.length > 1) {
       ...
       separator.textContent = `${component.length} düğüm ve ${numberOfLinks} bağlantıdan oluşan ağ`;
       ...
       sortedNodes.forEach((node) => {
        ...
        resultDiv.innerHTML = `<span class=\"shortname\" style=\"background-color: ${getShortnameColor(node.role, node.region)};\">${node.short_name}</span> ${node.long_name} <span style=\"color: #ff0;\">${node.id.toString(16).padStart(8, '0').slice(-4)}</span>`;
     ...
     if(singleNodes.length > 0) {
      ...
      separator.className = "separator";
      separator.setAttribute("data-entete", "true");
      separator.textContent = `${singleNodes.length} düğümün komşusu yok`;
      resultsDiv.appendChild(separator);
   ...
   <div style="border-radius: 1rem; position: absolute; right: 5px; bottom: 5px; color: white; background: rgba(0,0,0,0.7); padding: 10px; font-family: Arial, sans-serif; line-height: 1.6; text-align: left;">
  <span style="display:block; color:#0a0;">CLIENT EU_433</span>
  <span style="display:block; color:#FF9933;">CLIENT EU_868</span>
  <span style="display:block; color:#00f;">CLIENT MUTE</span>
  <span style="display:block; color:#80f;">ROUTER</span>
  <span style="display:block; color:#888;">IP GATEWAY</span>
</div>
 </body>
</html>
